...
def process_chunk(start_idx, end_idx):
    ...
    mesh = np.meshgrid(np.arange(cam_size_y + 1), np.arange(cam_size_x + 1), indexing='ij')
    y_mesh = mesh[0].ravel()
    x_mesh = mesh[1].ravel()
    grayscale = np.zeros((cam_size_y + 1, cam_size_x + 1), dtype=np.uint8)
    hist = np.zeros((cam_size_y + 1, cam_size_x + 1, hist_size), dtype=np.int64)
    ...
    for image_idx in range(start_idx, end_idx):
        ...
        hist_prefix[generic_image_id][:] = full_prefix[0:cam_size_y + 1:available_coords, 0:cam_size_x + 1:available_coords, :]

...
def init():
    ...
    #                             GradCAMPlusPlus, HiResCAM, LayerCAM, RandomCAM)
    import shelve
    # import time
    ...
    # model = resnet18()
    # model.load_state_dict(torch.load(f"/home/ubuntu/data/torch_models/resnet18/baseline/9.pth"))
    ...
    object_detection_map = load_object_region_index_in_memory(dataset_examples, '/data/explain_imagenet/shelves/object_detection_map.shelve')
...
def init_load_index(filename):
    ...
    # in_memory_index = np.load(f'/data/explain_imagenet/npy/monolithic_imagenet_cam_hist_prefix_{hist_size}_in_memory_available_coords_{available_coords}.npy')
    # in_memory_index_suffix = np.cumsum(in_memory_index[:, :, :, ::-1], axis=3)[:, :, :, ::-1]

    # del in_memory_index
    # np.save(f'/data/explain_imagenet/npy/monolithic_imagenet_cam_hist_prefix_{hist_size}_in_memory_available_coords_{available_coords}_suffix.npy', in_memory_index_suffix)
...
def init_incremental_indexing():
    global in_memory_index_suffix
    in_memory_index_suffix = np.zeros((total_images, (cam_size_y // available_coords) + 1, (cam_size_x // available_coords) + 1, hist_size), dtype=np.int64)